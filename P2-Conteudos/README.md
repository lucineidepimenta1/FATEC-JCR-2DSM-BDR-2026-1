
📢 **AVISO – PROVA 2 (Banco de Dados Relacional)**

Prezados estudantes,

Informo que a **Prova 2** da disciplina de **Banco de Dados Relacional** será realizada conforme as informações abaixo:

📅 **Data:** 16/06/2026 (terça-feira)
🕘 **Horário:** 21h25 às 23h05

### 📚 Conteúdo da Prova

A avaliação contemplará os conteúdos estudados nas seguintes aulas:

✅ **Aula 11 – Subconsultas (Subqueries)**

* Subconsultas escalares
* Subconsultas em cláusulas WHERE
* Uso de IN, EXISTS e operadores relacionais
* Aplicações práticas de subconsultas

✅ **Aula 13 – Views**

* Conceito e finalidade das Views
* Criação e utilização de Views
* Vantagens e limitações
* Aplicações em consultas e relatórios

✅ **Aula 14 – Stored Procedures**

* Conceito de Procedures
* Criação e execução de Procedures
* Parâmetros de entrada
* Boas práticas de utilização
* Vantagens e limitações

✅ **Aula 16 – Triggers**

* Conceito e funcionamento das Triggers
* Eventos BEFORE e AFTER
* Uso de OLD e NEW
* Automação de regras de negócio
* Auditoria e controle de integridade

### 📝 Orientações Importantes

* A prova será **individual**.
* A prova será **sem consulta**.
* A avaliação será realizada **em papel (prova escrita)**.
* Levar:

  * Caneta azul ou preta;
  * Lápis;
  * Borracha.

### ✍️ Estrutura da Avaliação

A prova conterá:

* Questões de múltipla escolha;
* Questões dissertativas;
* Análise e interpretação de trechos SQL;
* Situações-problema envolvendo os conteúdos estudados.

### ⏰ Horários

* Não haverá tempo adicional para estudantes que chegarem após o início da avaliação (até às 21:35).
* Será permitida a entrada de estudantes até **21h35**.
* Após esse horário, não será permitida a entrada de estudantes para realização da avaliação.
* A entrega da prova e saída da sala serão permitidas somente a partir das **22h30**.
* Ao entregar a prova, o estudante deverá deixar a sala de forma silenciosa para não prejudicar os colegas que ainda estiverem realizando a avaliação.

📌 Recomenda-se revisar os exemplos trabalhados em sala, exercícios resolvidos, atividades práticas individuais e os materiais disponibilizados pela professora.

Bons estudos e sucesso na avaliação!

### Profª Lucineide.
